...
impl JsReplManager {
    async fn execute(
        ...
        let (registry, hosted_specs) =
            crate::session::tests::tool_registry_for_test_step(&step_context);
        let router = Arc::new(crate::tools::ToolRouter::from_registry(
            &step_context.turn,
...
fn set_danger_full_access(turn: &mut crate::session::turn_context::TurnContext) {
    for environment in &mut turn.environments.environments {
        if let TurnEnvironmentState::Ready(environment) = environment {
...
fn set_turn_cwd(turn: &mut crate::session::turn_context::TurnContext, cwd: AbsolutePathBuf) {
    let mut primary = turn
        .environments
        .primary()
        .cloned()
        .expect("test turn should have a ready environment");
    primary.selection.cwd = PathUri::from_abs_path(&cwd);
    turn.environments.environments[0] = TurnEnvironmentState::Ready(primary);
    Arc::make_mut(&mut turn.config).cwd = cwd;
}

fn turn_cwd(turn: &crate::session::turn_context::TurnContext) -> AbsolutePathBuf {
    turn.environments
        .primary()
        .expect("test turn should have a ready environment")
        .cwd()
        .to_abs_path()
        .expect("test environment should use a native cwd")
}

fn shell_environment_policy_mut(
    turn: &mut crate::session::turn_context::TurnContext,
) -> &mut ShellEnvironmentPolicy {
    &mut Arc::make_mut(&mut turn.config)
        .permissions
...
async fn emitted_image_content_item_allows_explicit_original_detail_when_supported() {
    ...
    crate::session::tests::update_turn_settings_for_test(&mut turn, |settings| {
        Arc::make_mut(&mut settings.model_info).supports_image_detail_original = true;
    });
...
async fn interrupt_turn_exec_clears_matching_submitted_exec() -> anyhow::Result<()> {
    ...
    let mut state = manager
        .start_kernel(step_context, &dependency_env, /*thread_id*/ None)
        .await
...
async fn js_repl_waits_for_unawaited_tool_calls_before_completion() -> anyhow::Result<()> {
    ...
    let marker =
        turn_cwd(turn.as_ref()).join(format!("js-repl-unawaited-marker-{}.txt", Uuid::new_v4()));
...
async fn js_repl_persisted_tool_helpers_work_across_cells() -> anyhow::Result<()> {
    ...
    let global_marker =
        turn_cwd(turn.as_ref()).join(format!("js-repl-global-helper-{}.txt", Uuid::new_v4()));
    let lexical_marker =
        turn_cwd(turn.as_ref()).join(format!("js-repl-lexical-helper-{}.txt", Uuid::new_v4()));
...
async fn js_repl_prefers_env_node_module_dirs_over_config() -> anyhow::Result<()> {
    ...
    let (session, mut turn) = make_session_and_context().await;
    shell_environment_policy_mut(&mut turn).r#set.insert(
        "CODEX_JS_REPL_NODE_MODULE_DIRS".to_string(),
        env_base.path().to_string_lossy().to_string(),
    );
    set_turn_cwd(&mut turn, cwd_dir.abs());
...
async fn js_repl_resolves_from_first_config_dir() -> anyhow::Result<()> {
    ...
    let (session, mut turn) = make_session_and_context().await;
    shell_environment_policy_mut(&mut turn)
        .r#set
...
async fn js_repl_imported_local_files_can_access_repl_globals() -> anyhow::Result<()> {
    ...
    shell_environment_policy_mut(&mut turn)
        .r#set
        .insert("HOME".to_string(), "/tmp/codex-home".to_string());
    shell_environment_policy_mut(&mut turn)
        .r#set
        .remove("CODEX_JS_REPL_NODE_MODULE_DIRS");
    set_turn_cwd(&mut turn, cwd_dir.abs());

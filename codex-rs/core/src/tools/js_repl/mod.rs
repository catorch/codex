...
impl JsReplManager {
    ...
    async fn start_kernel(
        ...
        let exec_env = sandbox
            .transform(SandboxTransformRequest {
                ...
                environment_id: Some(turn_environment.environment_id.as_str()),
    ...
    async fn run_tool_request(exec: ExecContext, req: RunToolRequest) -> RunToolResult {
        ...
        let mcp_tools = exec.step_context.mcp_tools().await.to_vec();
        ...
        let (tool_call_name, payload) = if let Some(tool_info) = mcp_tools
            .iter()
            .find(|tool| tool.canonical_tool_name() == requested_tool_name)
        {
            (

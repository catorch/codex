...
impl JsReplManager {
    ...
    async fn start_kernel(
        ...
        let turn_environment = step_context
            .environments
            .primary()
            .ok_or_else(|| "js_repl is unavailable without a ready environment".to_string())?;
        if turn_environment.environment.is_remote() {
            return Err("js_repl is unavailable in a remote environment".to_string());
        }
        let cwd = turn_environment.cwd().clone();
        let native_cwd = cwd
            .to_abs_path()
            .map_err(|err| format!("js_repl cwd `{cwd}` is not native to the Codex host: {err}"))?;
        ...
        let exec_env = sandbox
            .transform(SandboxTransformRequest {
                ...
                environment_id: Some(turn_environment.selection.environment_id.as_str()),
                network: turn.network.as_ref(),
            ...
            .map_err(|err| format!("failed to prepare js_repl execution: {err}"))?;
        ...
        let exec_cwd = exec_env
            .cwd
            .to_abs_path()
            .map_err(|err| format!("js_repl execution cwd is not native: {err}"))?;
    ...
    async fn run_tool_request(exec: ExecContext, req: RunToolRequest) -> RunToolResult {
        ...
        let router = &exec.step_context.tool_router;
                    ...
                    namespace.tools.iter().find_map(|tool| match tool {
                        ResponsesApiNamespaceTool::Function(tool) => {
                            ...
                            (code_mode_name == req.tool_name
                                || tool_name.to_string() == req.tool_name)
                                .then_some(tool_name)
                        }
        ...
        let is_freeform = match router.find_spec(&requested_tool_name) {
            Some(ToolSpec::Freeform(_)) => true,
            Some(ToolSpec::Namespace(namespace)) => namespace.tools.iter().any(|tool| {
                matches!(tool, ResponsesApiNamespaceTool::Custom(tool)
                    if tool.name == requested_tool_name.name)
            }),
        ...
        let (tool_call_name, payload) = if is_freeform {
            (
                requested_tool_name,
...
fn emitted_image_content_item(
    ...
    FunctionCallOutputContentItem::InputImage {
        image_url,
        detail: normalize_output_image_detail(model_info, detail).or(Some(DEFAULT_IMAGE_DETAIL)),
    }
}

...
fn assert_projected_fields_match_policy(
    ...
    let policy = permission_profile
        ...
        .materialize_project_roots_with_cwd(cwd.as_path());
...
#[test]
fn effective_workspace_permissions_bind_remaining_symbolic_root_to_profile_cwd() {
    let temp_dir = TempDir::new().expect("temp dir");
    let cwd = absolute_path(temp_dir.path());
    let effective =
        assert_projected_fields_match_policy(&PermissionProfile::workspace_write(), &cwd);

    assert_eq!(effective.can_write(cwd.join("src").as_path()), true);
    assert_eq!(effective.can_write(cwd.join(".codex").as_path()), false);
}

...
fn create_effective_filesystem_args(
    ...
    for writable_root in &sorted_writable_roots {
        ...
        let mut read_only_subpaths: Vec<PathBuf> = writable_root
            ...
            // Missing automatic metadata carveouts are enforced by the
            // protected-create path below rather than an eager placeholder.
            .filter(|path| {
                !effective_filesystem_permissions.is_missing_protected_metadata_carveout(path)
            })
            .collect();
